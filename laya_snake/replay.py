...
class TerminalRaster:
    def __init__(self, columns, rows, *, width=1920, height=1080, font=None):
        choices = [Path(font)] if font else [Path(p) for p in FONTS]
...
def main(argv=None):
    parser = argparse.ArgumentParser(prog="laya-snake export", description=__doc__,
                                     formatter_class=argparse.RawDescriptionHelpFormatter)
    parser.add_argument("recording", type=Path, help="A JSONL file written by laya-snake --record")
    ...
    if not ffmpeg:
        parser.error("MP4 export needs ffmpeg on PATH")
