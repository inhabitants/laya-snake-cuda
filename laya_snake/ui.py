...
def status_line(stats, t):
    """The disturb-mode status as (text, color), or (None, None) when there is nothing to say."""
    if stats.get("driving"):
        return t["driving"], RED
    status = stats.get("status") or {}
    kind = status.get("kind")
    if kind == "recovering":
        return t["recovering"], AMBER
    if kind == "recovered":
        return t["recovered"].format(n=status["moves"]), GREEN
    if kind == "fell":
        reason = t["reasons"].get(status.get("reason"), status.get("reason"))
        return t["fell"].format(reason=reason), RED
    return None, None


def compose(game, decision, stats):
    """Probabilities describe the displayed board, before its announced next step."""
    t = text(stats.get("lang", "en"))
    ...
    if stats.get("flash_text"):
        c.put(4, left + 41, stats["flash_text"][:12], RED)
    ...
    c.put(4, right, "Laya", GREEN)
    c.put(5, right, f"{stats.get('hardware', 'GPU')} · Local", MUTED)
    ...
    if decision.get("pushed"):
        c.put(14, right + 20, t["you_tag"], RED)
    ...
    c.put(26, right + 18, f"{stats.get('engine', 'CUDA')} · {stats.get('precision', '')}", MUTED)
    ...
    mode = game.get("mode")
    if mode:
        c.put(30, right, t["cycle"] if mode == "cycle" else t["free"], GREEN if mode == "cycle" else AMBER)
    line, tone = status_line(stats, t)
    if line:
        c.put(30, right + 12, line[: side - 12], tone)
    c.put(
        31,
        right,
        t["counters"].format(
            a=stats.get("takeovers", 0), b=stats.get("recoveries", 0), c=stats.get("falls", 0)
        ),
        MUTED,
    )
    c.put(height - 3, left, "─" * (width - 6), DIM)
    c.put(height - 2, left, t["keys"], MUTED)
    c.put(height - 1, left, t["keys2"], MUTED)
